...
const BuilderBlockProperties = () => {
  const { selectedBlockLayout, blockLayouts } = useBuilder();

  const LayoutPropertyBlock =
    selectedBlockLayout &&
    FormBlocks[selectedBlockLayout.blockType]?.propertiesComponent;

  // Get position index for the selected block
  const positionIndex = selectedBlockLayout 
    ? blockLayouts.findIndex(block => block.id === selectedBlockLayout.id) + 1
    : undefined;
            ...
            <div className="w-full pt-1">
              ...
              >
                <h5
                 ...
                 font-medium text-sm"
                >
                  {selectedBlockLayout?.blockType === 'RowLayout' 
                ...
                {LayoutPropertyBlock && (
                  <LayoutPropertyBlock 
                    ...
                    parentId={undefined} // Top-level blocks don't have parents
                  />
                )}
              </div>
            </div>
